...
### D1 — C2 NoteStore folded into C1 CLI

Milestone: M1
Change: `note/store.py` removed. `storage_path()` and the append/read logic live
in `note/cli.py`; there is no separate store module or C1 → C2 boundary.
Reason: C2's entire surface was three functions with no logic of their own beyond
file access, and no second caller ever materialised. The agreed boundary bought
no testability (the tests drive the CLI through real process runs via `NOTE_FILE`,
not the store API) and no substitutability (the requirements' `Non-Goals` rule out
sync and multi-user, which were the only plausible second backends). Keeping it
would have been an abstraction with no current use.
Material: yes — this removes an agreed component and its boundary. Confirmed with
the human before M1 completion.
